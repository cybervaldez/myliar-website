// /auditions-v1/<campaign>/<step> — one campaign's audition for one step. Campaign-primary: the clean
...
export default async function CampaignStepPage({ params }: { params: Promise<{ campaign: string; step: string }> }) {
  ...
  if (!c || !sd) return <main style={{ maxWidth: 760, margin: "0 auto", padding: 40 }}><p style={{ color: "var(--margin-ink)" }}>No audition for {campaign} · {step}. <a href="/auditions-v1" style={{ color: "var(--forest)" }}>↑ the board</a></p></main>;
  ...
  const prev = at > 0 ? { href: `/auditions-v1/${campaign}/${order[at - 1].key}`, label: `${stepNo(order[at - 1].key)} ${order[at - 1].label.replace(/^The /, "")}` } : { href: `/auditions-v1/${campaign}/`, label: c.label };
  const next = at < order.length - 1 ? { href: `/auditions-v1/${campaign}/${order[at + 1].key}`, label: `${stepNo(order[at + 1].key)} ${order[at + 1].label.replace(/^The /, "")}` } : null;
