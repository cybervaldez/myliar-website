// /auditions-v1/<campaign> — one story's SPINE: its pipeline, step by step, born from the shared slate.
...
export default async function CampaignSpine({ params }: { params: Promise<{ campaign: string }> }) {
  ...
  if (!c) return <main style={{ maxWidth: 760, margin: "0 auto", padding: 40 }}><p style={{ color: margin }}>Unknown story. <a href="/auditions-v1" style={{ color: forest }}>↑ the board</a></p></main>;
  ...
  return (
    <main style={{ maxWidth: 760, margin: "0 auto", padding: "24px 20px 80px" }}>
      ...
      <p style={{ fontSize: 11, color: margin, margin: "0 0 20px" }}>born from <a href="/auditions-v1/concept" style={{ color: forest }}>the slate</a> · the pipeline runs destination-backward, each step carrying the last one's experts forward.</p>

      {order.map((s, k) => (
        <div key={s.key}>
          {s.done ? (
            <a href={`/auditions-v1/${campaign}/${s.key}`} style={{ display: "block", textDecoration: "none", border: `2px solid var(--ink-soft)`, background: paper, padding: "12px 15px", color: ink }}>
