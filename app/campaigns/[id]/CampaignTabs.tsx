...
function MermaidFlow({ web, arc, uid }: { web: string; arc: string; uid: string }) {
  ...
  useEffect(() => {
    ...
    if (!src.trim()) { if (ref.current) ref.current.innerHTML = ""; return; }
    let cancelled = false;
    (async () => {
      try {
        // runtime CDN import — kept out of the bundle so static pages stay light
        ...
        const { svg } = await mermaid.render(`mer-${uid}-${mode}`, src);
  ...
  return (
    <div>
      ...
      <p className="text-[12px] text-margin-ink mb-3 leading-[1.5]">
        Whole-campaign. <b>Callback web</b> = the selection→future-dialogue graph (a flag set on one day,
        read on a later day) — the Influence map made visual. <b>Arc interleave</b> = the full coach weave;
        ★ = Unspoken, dashed = a callback, colors = coach.
      </p>
      {mode === "web" && !web.trim() ? (
