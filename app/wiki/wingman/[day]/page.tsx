// Wingman day page — the full structured canon for one day of the dating
// expansion, generated from the run-wingman payload. Every event, choice,
// stat delta, reaction, memory write, and reveal, with frame-flags surfaced.
// Spoilers visible (the internal review surface). Discussion anchors are
// namespaced `wingman:*` so they never collide with the Main Line arc.
...
export function generateStaticParams() {
  return wingmanDays().map((d) => ({ day: String(d.globalDayIndex) }));
}
...
export async function generateMetadata({
  ...
  const d = wingmanDay(Number(day));
  if (!d) return { title: "Unknown Day — Wiki" };
  const focal = d.characterId ? wingmanCoachById(d.characterId) : null;
  return {
    title: `Day ${d.globalDayIndex}${focal ? ` · ${focal.name}` : ""} — The Wingman`,
    description: `Day ${d.globalDayIndex} of the Wingman arc (${d.narrativeType}).`,
  };
}
    ...
    <div className="border-[1.5px] border-ink bg-paper p-3">
      ...
      {c.itemDrop && (
        <div className="border border-forest bg-paper-shade p-2 mt-2">
          ...
          <DiscussionThread
            anchor={`wingman:item:${c.itemDrop.name}`}
      ...
      <DiscussionThread
        anchor={`wingman:day-${day}:event:${eventId}:reply:${c.id}`}
...
export default async function WingmanDayPage({
  params,
}: {
  params: Promise<{ day: string }>;
}) {
  ...
  const infobox = (
    <Infobox
      ...
      rows={[
        ...
        ...(focal?.statLane ? [{ label: "LANE", value: focal.statLane }] : []),
  ...
  const navbox = (
    <Navbox
      title="THE WINGMAN"
      links={wingmanDays().map((x) => ({
        label: `Day ${x.globalDayIndex}`,
        href: `/wiki/wingman/${x.globalDayIndex}`,
      }))}
    />
  );
  ...
  return (
    <WikiPage
      ...
      breadcrumb={[
        ...
        { label: "The Wingman", href: "/wiki/wingman" },
      ]}
      infobox={infobox}
      navbox={navbox}
    >
      ...
      {d.events.map((ev, idx) => {
        ...
        return (
          <div key={ev.id} className="mb-7">
              ...
              <div className="mt-3 border-l-2 border-forest pl-3">
                ...
                {ev.memoryWrites.map((m, i) => (
                  <div key={i} className="mb-1.5">
                    ...
                    <DiscussionThread
                      anchor={`wingman:day-${d.globalDayIndex}:event:${ev.id}:memory:${i}`}
            ...
            <DiscussionThread
              anchor={`wingman:day-${d.globalDayIndex}:event:${ev.id}`}
      ...
      <DiscussionThread
        defaultOpen
        anchor={`wingman:day-${d.globalDayIndex}`}
