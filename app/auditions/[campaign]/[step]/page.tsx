...
export default async function CampaignStepPage({ params }: { params: Promise<{ campaign: string; step: string }> }) {
  ...
  if (step === "scenes") {
    ...
    const rd: RangeT | null = sc?.premises && sc?.range ? { premises: sc.premises, range: sc.range, coverage: sc.coverage, gatedBy, rangeReview: sc.rangeReview, expertsGate: sc.expertsGate, branches, honing: sc.honing, ensemble: sc.ensemble, interaction: sc.interaction, relCoverage: sc.relCoverage, floors: sc.floors, sensory: sc.sensory, narrative: sc.narrative?.scenes, narrativeSpread: sc.narrative?.spread } : null;
    return (
      <main className="aud-main" style={{ padding: "24px 20px 80px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", fontSize: 12, marginBottom: 4 }}>
          <a href="/auditions" style={{ color: "var(--margin-ink)", textDecoration: "none" }}>↑ the board</a>
