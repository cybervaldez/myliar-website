...
export default async function CampaignStepPage({ params }: { params: Promise<{ campaign: string; step: string }> }) {
  ...
  let prepend = null;
  if (step === "pilot") {
    const raw = c.steps.pilot as unknown as { scenes: string[]; scrubGroups: { id: string; name: string; settingTitle: string; storyTitles: string[]; throughline: string; env: string[]; buildingBlock: string; verdict?: string; metaphor?: string; audienceServe?: string }[]; worldbuild?: { richest: string; note: string } };
    ...
    prepend = <Scrubber scenes={raw.scenes} groups={groups} richest={rich} note={raw.worldbuild?.note} />;
  }
