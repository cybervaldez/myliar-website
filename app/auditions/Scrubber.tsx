...
export type RangeT = {
  ...
  interaction?: { map: { scene: string; engine?: string; family?: string; gate?: string; gesture?: string; graduates?: string; cg?: string; motion?: string }[]; range?: { variety?: string; density?: string; enough?: string; gaps?: string; note?: string }; flags?: { scene: string; issue: string; resolution: string }[] };
};
          ...
          <div style={{ padding: "2px 13px 12px", fontSize: 11, color: ink, lineHeight: 1.5 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "minmax(70px,auto) auto 1fr", gap: "5px 10px", fontSize: 10.5 }}>
              ...
              {d.interaction.map.map((m) => (
                <Fragment key={m.scene}>
                  <div style={{ fontWeight: 700, color: forest, textTransform: "capitalize" }}>{m.scene}</div>
                  <div><span title={m.gate} style={{ fontFamily: "var(--theme-body)", fontSize: 8.5, fontWeight: 700, color: m.family === "living" ? forest : m.family === "contest" ? amber : margin, border: `1px solid ${m.family === "living" ? forest : m.family === "contest" ? amber : "var(--ink-soft)"}`, borderRadius: 3, padding: "0 5px", whiteSpace: "nowrap" }}>{m.engine}</span>{m.family === "contest" ? <span style={{ fontSize: 8, color: amber, marginLeft: 3 }}>⚡</span> : null}</div>
    ...
    <div>
      ...
      {b.interaction && (b.interaction.engine || b.interaction.cg) && (
        <>
          ...
          <div style={{ border: `2px solid ${forest}`, background: shade, padding: "9px 12px", marginBottom: 16, fontSize: 12, color: ink, lineHeight: 1.5 }}>
            {b.interaction.engine && <div style={{ marginBottom: 4 }}><span style={{ fontFamily: "var(--theme-body)", fontSize: 8.5, fontWeight: 700, color: b.interaction.family === "living" ? forest : b.interaction.family === "contest" ? amber : margin, border: `1px solid ${b.interaction.family === "living" ? forest : b.interaction.family === "contest" ? amber : "var(--ink-soft)"}`, borderRadius: 3, padding: "1px 6px" }}>{b.interaction.engine}</span>{b.interaction.family && b.interaction.family !== "none" ? <span style={{ fontSize: 9, color: b.interaction.family === "contest" ? amber : margin, marginLeft: 4 }}>{b.interaction.family === "contest" ? "⚡ chaotic-pick gamble" : "living · REL-gated"}</span> : null} {b.interaction.gesture && b.interaction.gesture !== "—" ? <span>{b.interaction.gesture}</span> : <span style={{ color: margin, fontStyle: "italic" }}>pure perception + choice (no minigame)</span>}</div>}
